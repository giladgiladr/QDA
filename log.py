...
class LogRecord(object):
    ...
    def __init__(self, log_size=HEADER_SIZE, type=RECORD_TYPE_NONE, message_id=0, timestamp=0, flags=None):
    ...
    def decode(self, data):
        ...
        self.log_size, self.type, self.message_id, self.timestamp, self.flags = struct.unpack("<QQQQQ", data[
                                                                                                        0:LogRecord.HEADER_SIZE])
    ...
    def dump(self):
        ret = ""
        ret += "TODO: CLASS NAME(0x%x): id=0x%x, ts=0x%x, log_size=0x%x flags=%d\n" % (self.type,
                                                                                       self.message_id,
                                                                                       self.timestamp,
                                                                                       self.log_size,
                                                                                       self.flags)
...
class LogTypes(object):
    ...
    def __init__(self):
        ...
        if LogTypes.log_types == None:
            LogTypes.log_types = self
        else:
            raise
...
class LogManager(object):
    ...
    def __init__(self, log_folder):
        ...
        idx_path = os.path.join(log_folder, LOG_INDEX_FILE_NAME)
        if QFS.exists(log_path):
            mode = "r+b"
        else:
            mode = "wb"
        self.log_file = QFS.open(log_path, mode)
        self.log_idx = QFS.open(idx_path, mode)
